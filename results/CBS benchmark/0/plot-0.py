...
import numpy as np
b3lyp_dp = np.loadtxt('linear-0-B3LYPD3BJ-datapoints.txt')
b3lyp_sp = np.loadtxt('linear-0-B3LYPD3BJ-spline.txt')
cbs_dp = np.loadtxt('linear-0-cbs-datapoints.txt')
cbs_sp = np.loadtxt('linear-0-cbs-spline.txt')
ccsdt_dp = np.loadtxt('linear-0-ccsd(t)-tz-datapoints.txt')
ccsdt_sp = np.loadtxt('linear-0-ccsd(t)-tz-spline.txt')
dsd_dp = np.loadtxt('linear-0-DSDPBEP86D3BJ-datapoints.txt')
dsd_sp = np.loadtxt('linear-0-DSDPBEP86D3BJ-spline.txt')
sapt0_dp = np.loadtxt('linear-SAPT0-0-datapoints.txt')
sapt0_sp = np.loadtxt('linear-SAPT0-0-spline.txt')
b3lyp_dp_r = b3lyp_dp[:,0]
b3lyp_dp_e = b3lyp_dp[:,1]
...
cbs_sp_r = cbs_sp[:,0]
cbs_sp_e = cbs_sp[:,1]
ccsdt_dp_r = ccsdt_dp[:,0]
ccsdt_dp_e = ccsdt_dp[:,1]
...
dsd_sp_r = dsd_sp[:,0]
dsd_sp_e = dsd_sp[:,1]
sapt0_dp_r = sapt0_dp[:,0]
sapt0_dp_e = sapt0_dp[:,1]
...
figure_fig = plt.gcf()
figure_fig.savefig('0-benchmark.pdf', format='pdf', dpi=1000)